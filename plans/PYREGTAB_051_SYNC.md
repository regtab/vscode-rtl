...
**Статус:** ВЫПОЛНЕН (0.8.9; публикация не запускалась)
**Дата:** 2026-08-26
...
Выполнено полностью, все пункты §3-§8 закрыты.

**Проверки** (все зелёные):

```
cargo clippy --all-targets -- -D warnings   чисто
cargo test                                  26 unit (было 25) + 3 corpus, 0 failed
npm test                                    check:types + test:grammar + 39 unit + check:sync
npm run test:e2e                            5 passing
npx vsce package                            114.58 КБ (лимит CI — 200 КБ)
```

**Дискриминирующая проверка нового теста пройдена.** На временно откаченном
пине `v0.5.0` тест падает ровно на утверждении о схеме:

```
left:  ["Lokaler", "Dato", "Klasse"]
right: ["Dato", "Lokaler", "Klasse"]
```

Значения при этом совпадают на обоих ядрах — то есть тест ловит именно
разъехавшуюся связку имя ↔ значение, а не что-то попутное. Пин восстановлен.

**Предсказание по hover подтвердилось буквально:** `git diff --stat` дал
`1 file changed, 2 insertions(+), 2 deletions(-)` — ровно записи `ANCH` и
`REC(n)`. Проза раздела «Settings prefix» и блок `!!! note` в словарь не
попали, позиционные индексы таблиц не перекосило.

**Ни один эталон не подогнан под старое поведение:**

- `git status` по `syntaxes/`, `grammar/RTL.g4`, `server/tests/corpus/` — пусто;
- строка `sha256:` в `grammar/UPSTREAM` не менялась (в дифе только `commit:`
  и `tag:`);
- все 11 существующих `*.snap` не изменились — добавились только два новых;
- `negative_corpus_positions` зелёный, то есть `conflict_anch_rec.rtl`
  по-прежнему даёт диагностику.

**Изменения пинов — явный список:**

| Файл | Было | Стало |
|---|---|---|
| `server/Cargo.toml:20` | `tag = "v0.5.0"` | `tag = "v0.5.1"` |
| `server/Cargo.lock:438-439` | `0.5.0`, `#abca0c7f…` | `0.5.1`, `#0eb6ea50…` |
| `grammar/UPSTREAM:1-2` | `035ff1a…` / `v0.5.0` | `68be345…` / `v0.5.1` |
| `server/src/main.rs:25` | `jRegTab 0.5.0` | `jRegTab 0.5.1` |
| `README.md:159`, `CLAUDE.md:35` | `jRegTab 0.5.0` | `jRegTab 0.5.1` |
| `CLAUDE.md:40`, `DEVELOPMENT.md:15` | `v0.5.0` | `v0.5.1` |
| `package.json`, `package-lock.json` | `0.8.8` | `0.8.9` |

**Отклонения от плана (два, оба мелкие):**

1. §5 предлагал `description` со словом «normalisation». В коде расширения
   принято американское написание (`normalized` в `README.md:78`,
   `server/src/main.rs:600`, `CHANGELOG.md:128`); британское встречается только
   в генерируемом `hover_data.rs`, куда оно приезжает из pyregtab. Оставлено
   `normalization`, изменена только вводящая в заблуждение часть:
   «anchor column» → «anchor attribute position».
2. `package-lock.json` тоже несёт версию (две строки) — обновлён вслед за
   `package.json`, как и в релизе 0.8.8.

**Наблюдение, не требующее правок.** В снапшоте `anch_named_attrs.rtl.snap`
токен `ANCH` получает скоуп `support.function.extractor.rtl`: правило
`syntaxes/rtl.tmLanguage.json:177` держит настройки и строковые экстракторы в
одном классе. Подсветка есть (в общий текст не проваливается), это
предсуществующее решение модели скоупов, а не регрессия этого синка.
